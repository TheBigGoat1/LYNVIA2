...
export async function POST(request: NextRequest) {
  try {
      ...
      line_items: [
        {
          price_data: {
            currency: 'chf',
            product_data: { name: 'Scenario — additional analysis (Lynvia)' },
            unit_amount: amountCents,
          },
          quantity: 1,
        },
      ],
      success_url: `${baseUrl}/individual/scenario-calculator/success?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${baseUrl}/individual/scenario-calculator?scenario_cancel=1`,
      metadata: {
        userId: decoded.uid,
        purpose: 'scenario_addon',
    ...
    console.error('scenario-checkout', err);
