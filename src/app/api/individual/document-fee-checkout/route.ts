...
const bodySchema = z.object({
  ...
  purpose: z.enum(['document_review', 'document_pdf']),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { amountCents, purpose, locale } = parsed.data;
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:9002';
    const loc = locale || 'en';

    const label =
      purpose === 'document_review'
        ? 'Document — submit for review (Lynvia)'
        : 'Document — PDF download (Lynvia)';
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${baseUrl}/${loc}/individual/document-generator/success?fee_purpose=${purpose}&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${baseUrl}/${loc}/individual/document-generator?fee_cancel=1`,
      metadata: {
        userId: decoded.uid,
        purpose,
    ...
    console.error('document-fee-checkout', err);
