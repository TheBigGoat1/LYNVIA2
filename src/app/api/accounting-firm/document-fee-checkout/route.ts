...
export async function POST(request: NextRequest) {
  try {
      ...
      line_items: [
        {
          price_data: {
            currency: 'chf',
            product_data: { name: 'Document — PDF/Word download (Lynvia Fiduciaire)' },
            unit_amount: amountCents,
          },
          quantity: 1,
        },
      ],
      success_url: `${baseUrl}/${loc}/accounting-firm/document-generator/success?fee_purpose=${purpose}&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${baseUrl}/${loc}/accounting-firm/document-generator?fee_cancel=1`,
      metadata: {
        userId: decoded.uid,
    ...
    console.error('accounting-firm/document-fee-checkout', err);
